...
final public class HMAC {
    ...
    class public func authenticate(key  key: [Byte], message: [Byte], variant:HMAC.Variant = .md5) -> [Byte]? {
        return HMAC(key, variant: variant)?.authenticate(message: message)
    }
    
    // MARK: - Private
    
    public init? (_ key: [Byte], variant:HMAC.Variant = .md5) {
    ...
    public func authenticate(message  message:[Byte]) -> [Byte]? {
